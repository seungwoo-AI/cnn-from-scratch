# code/train_adam.py
"""
AdamCNN  (ReLU + Conv3 + Adam) 학습 스크립트
───────────────────────────────────────────────
* 모델  : EnhancedCNN  (Conv3×3→Conv3×3→Conv2×2, ReLU, 풀링 없음)
* 옵티마이저 : Adam     (lr=1e-3, β1=0.9, β2=0.999, eps=1e-8)
* 에폭  : 300
* 데이터 : 6×6 손글씨 (96개)  ─ train 80 %, valid 20 %
"""

import numpy as np
from utils.util_excel import load_excel_dataset
from models.model import EnhancedCNN          # ← ReLU + 3-Conv 스택
from nn.optimizer import Adam
from nn.activation import softmax, cross_entropy_loss, cross_entropy_grad


def collect_params(model):
    """Conv2D 레이어의 (파라미터, grad) 튜플을 자동 수집."""
    params = []
    for name in dir(model):
        layer = getattr(model, name)
        if hasattr(layer, "W"):                  # Conv2D라면
...
def train(seed=0, lr=1e-3):
    ...
    # 3) 모델 & 옵티마이저
    model = EnhancedCNN()
    optimizer = Adam(collect_params(model),
                    lr=5e-4, beta1=0.9, beta2=0.99, eps=1e-8)

    # 4) 학습 루프
    for epoch in range(300):
        p = softmax(model.forward(X_tr))
        loss = cross_entropy_loss(p, y_tr)

        model.backward(cross_entropy_grad(p, y_tr))
        optimizer.step()
        optimizer.zero_grad()
